...
function render(data) {
  ...
  q("system").innerHTML = [
    ...
    row("OBJECTIVE", sys.objective || "?"),
    row("GUARDRAILS", sys.guardrails || "?"),
  ...
  q("paper").innerHTML = [
    row("OPEN_POS", paper.open_positions ?? 0),
    ...
    row("TOTAL", fmtPnl(paper.total_pnl_cents), Number(paper.total_pnl_cents) >= 0 ? "good" : "bad"),
  ...
  const posList = Object.entries(positions).map(([ticker, p]) =>
    `${ticker}\n  ${p.side} x${p.qty} entry=${p.entry_price} mark=${p.mark_price} uPnL=${p.unrealized_pnl_cents}c`
  );
